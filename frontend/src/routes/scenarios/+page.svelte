<script lang="ts">
    ...
    import ScenarioScopeModal from "./components/ScenarioScopeModal.svelte";
    ...
    function isCurrentMonth(month: { periodStart?: string; periodEnd?: string }) {
        if (!month?.periodStart || !month?.periodEnd) return false;
        const start = new Date(month.periodStart);
        const end = new Date(month.periodEnd);
        const now = new Date();
                                            ...
                                            <tbody class="divide-y divide-slate-100 text-xs font-bold text-slate-600">
                                                {#each projectionResult.months as month, index}
                                                    <tr
                                                        ...
                                                        class="transition-all cursor-pointer group {isCurrentMonth(month) ? 'bg-orange-500/10 dark:bg-orange-500/15 hover:bg-orange-500/20 dark:hover:bg-orange-500/25 border-l-4 border-l-orange-500' : 'hover:bg-indigo-50/30 active:bg-indigo-50/60'}"
                                                    >
                                                        <td class="px-6 py-4 text-slate-950 dark:text-white font-black">
                                                            <span class={isCurrentMonth(month) ? "text-orange-600 dark:text-orange-400 font-extrabold" : "group-hover:text-indigo-600 transition-colors"}>
...
<ScenarioScopeModal
    ...
    bind:activeScenario={activeScenario}
    bind:scopeTab={scopeTab}
    ...
    deselectAllOfType={deselectAllOfType}
    toggleEntity={toggleEntity}
    toggleInRemainderOrder={toggleInRemainderOrder}
    moveInRemainderOrder={moveInRemainderOrder}
    getID={getID}
    getName={getName}
