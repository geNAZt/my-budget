...
*   **Bulk Import Standard:** Every data editor (Income, Bills, Expenses, etc.) MUST support a "Bulk Verification Engine":
    *   **Source:** Direct TSV/Clipboard paste.
    *   **Verification stage:** Show a preview table where users must "Accept" or "Ignore" detected entries before committing.
    *   **Locale Overrides:** Provide a toggle between "International (.)" and "German (,)" numeric parsing to handle legacy spreadsheet data.
...
*   **Directory Structure:**
    *   `web/backend/`: Go (Echo/DDD).
    *   `web/backend/cmd/`: Entry points.
    *   `web/backend/internal/domain/`: Pure business logic.
    *   `web/backend/internal/api/`: Handlers and routing.
    *   `web/backend/internal/db/`: Persistence layer.
...
*   **Directory Structure:** `web/frontend/`: SvelteKit + TailwindCSS 4.
...
1. **STATE: RESEARCH** * Check root directory for existing GAS classes to map equivalent Go logic.
   * Call `grep_search` on the target directory before reading files (Keep reads under 150 lines per call).
2. **STATE: PLAN** * Modify/Write to `web/docs/DESIGN.md` explaining the intended architecture. 
