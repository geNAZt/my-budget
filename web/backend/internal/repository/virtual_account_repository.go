...
func (r *VirtualAccountRepository) Save(userID string, va *domain.VirtualAccount) error {
	...
	v.VirtualAccountID = va.ID
	_, err = tx.Exec(`
		INSERT INTO virtual_account_versions (id, virtual_account_id, color, starting_balance, description)
		VALUES (?, ?, ?, ?, ?)`,
		v.ID, v.VirtualAccountID, v.Color, v.StartingBalance, v.Description)
	if err != nil {
		return err
	}
