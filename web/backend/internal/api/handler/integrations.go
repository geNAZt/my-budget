...
// SaveBulk automatically registers as "integrations::transactions::save_bulk"
func (t *IntegrationsTransactions) SaveBulk(s *api.WebsocketSession, reqID string, req *apiproto.TransactionSaveBulkRequest) {
	userID, _ := s.GetAuth()
	if userID == "" {
		t.handler.SendError(s, reqID, http.StatusUnauthorized, "Unauthorized")
		return
	...
	masterKey, err := t.syncService.GetMasterKey(userID, req.IntegrationId)
	if err != nil {
		t.handler.SendError(s, reqID, http.StatusInternalServerError, err.Error())
		return
	}

	var domainObjs []domain.BankTransaction
	now := time.Now()

	for _, txProto := range req.Transactions {
		parsedTime, err := time.Parse(time.RFC3339, txProto.CreatedAt)
		if err != nil {
			parsedTime = now
		}
		...
		genericTx := domain.GenericTransaction{
			...
			Description: txProto.Description,
			Peer:        txProto.Receiver,
			CreatedAt:   parsedTime,
			ExternalID:  txProto.Id,
		}
		if genericTx.ExternalID == "" {
			genericTx.ExternalID = uuid.New().String()
		}

		txJSON, _ := json.Marshal(genericTx)
		encryptedData, err := t.crypto.Encrypt(masterKey, txJSON)
		if err != nil {
			t.handler.SendError(s, reqID, http.StatusInternalServerError, err.Error())
			return
		}
		...
		sourceAcc := ""
		destAcc := req.AccountId
		if txProto.Amount < 0 {
			sourceAcc = req.AccountId
			destAcc = ""
		}

		domainObj := domain.BankTransaction{
			ID:                   uuid.New().String(),
			UserID:               userID,
	...
	if len(domainObjs) > 0 {
		if err := t.repo.SaveBulk(userID, domainObjs); err != nil {
			t.handler.SendError(s, reqID, http.StatusInternalServerError, err.Error())
			return
		}
	}

	t.handler.SendResponse(s, reqID, &apiproto.GenericID{Id: req.AccountId}, true)
}
